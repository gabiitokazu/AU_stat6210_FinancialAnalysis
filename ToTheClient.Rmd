...
stock1<- na.omit(ClCl(get(s[1])))
stock2 <- na.omit(ClCl(get(s[2])))
stock3<- na.omit(ClCl(get(s[3])))
stock4 <- na.omit(ClCl(get(s[4])))
stock5<- na.omit(ClCl(get(s[5])))

#varaince of stock
sigma_stocks <- var(cbind(stock1, stock2,stock3,stock4,stock5))
colnames(sigma_stocks)=c("stock1","stock2","stock3","stock4","stock5")

#diagonal elements of stock varaince
a=diag(sigma_stocks)
#first 3 minimum values
b=sort(a)[1:3]
#giving the corresponding stock
stock_1=which(a==b[1], arr.ind=TRUE)
stock_2=which(a==b[2], arr.ind=TRUE)
stock_3=which(a==b[3], arr.ind=TRUE)

```
here, stock `r stock_1`, stock`r stock_2` and stock`r stock_3` has smallest variance of change in  closing price.
