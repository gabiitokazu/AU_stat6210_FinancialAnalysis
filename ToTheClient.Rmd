...
Graphically, this is shown in the plot below. Your portfolio of choice is represented by the green dot, with the lowest risk. All other considered portfolios are represented by blue dots, with the one with the highest return in red, just for curiosity. The total risk of your best portfolio is `r best$Risk` and the daily expected return is `r best$ExpReturn`. The weight for each stock is `r best$FTV` for *Fortive Corp. Stock*, `r best$ZBH` for *Zimmer Biomet Holdings Inc. Stock*, and `r best$CTXS` for *Citrix Systems Inc. Stock*. In case you are interested, please refer to the table with the full results in the Appendix.
